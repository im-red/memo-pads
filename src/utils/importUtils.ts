...
export function exportData(selectedNotebookIds: string[], notebooks: Notebook[], memos: Memo[]) {
  const exportNotebooks = notebooks.filter(n => selectedNotebookIds.includes(n.id));
  const exportMemos = memos.filter(m => selectedNotebookIds.includes(m.notebookId));
  
  const data = JSON.stringify({
    notebooks: exportNotebooks,
    memos: exportMemos
  }, null, 2);
  ...
  a.download = `memo-pads_${date}.json`;
